...
const latestNotifications =
    require('../../customModules/freejobalerts/scraper').latestNotifications;
const topicScraper =
    require('../../customModules/freejobalerts/scraper').topicScraper;
const smartScraper =
    require('../../customModules/freejobalerts/scraper').smartScraper;
const stateCodes = require('../../data/freeJobAlertStateMap.json');
log.scope("freejobalert:global");

/////////////////////////////////////////////////// routes
/////////////////////////////////////////////////
router.get('/', getDefault, (req, res) => { res.send(res.results); });

router.get('/gov/other-all-india-exam', otherAllIndiaExam,
           (req, res) => { res.send(res.results); });

router.get('/gov/state/:code([a-zA-Z]{2})', stateWiseGovjobs,
           (req, res) => { res.send(res.results); })

router.get('/bank-jobs', bankJobs, (req, res) => { res.send(res.results); });

router.get('/teaching-jobs', allIndiaTeachingJobs,
           (req, res) => { res.send(res.results); });

router.get('/engineering-jobs', allIndiaEngineeringJobs,
           (req, res) => { res.send(res.results); });

router.get('/railway-jobs', allIndiaRailwayJobs,
           (req, res) => { res.send(res.results); });

router.get('/defence-jobs', allIndiaDefenceJobs,
           (req, res) => { res.send(res.results); });
...
router.all('*', notFound,
           (req, res) => { res.json({err : "404: invlid URL"}); })
...
async function stateWiseGovjobs(req, res, next) {
  ...
  var state =
      stateCodes.filter(function(item) { return item.code == stateCode; })[0];
  var URL = state.link;
  log.info(URL);
